        ...
        <TabsContent value={activeTab} className="mt-0">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredMarketplaces.map((marketplace) => (
              <Card
                key={marketplace.id}
                className="p-6 rounded-3xl border-slate-200 hover:border-blue-300 hover:shadow-lg transition-all bg-white"
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  <Link href={`/marketplace/${marketplace.id}`} className="w-full">
                    <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium py-2.5 cursor-pointer">
                      Подробнее
                    </Button>
                  </Link>
                </div>
